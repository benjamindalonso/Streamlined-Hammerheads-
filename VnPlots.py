...
def plot_vn(Weight):
    ...
    V_C = find_vmax(Weight)   # Max level flight speed from thrust=drag
    V_D = 1.13 * V_C          # Dive speed
    ...
    V_A_pos = np.sqrt(n_pos_limit / stall_coeff)
    V_A_neg = np.sqrt(abs(n_neg_limit) / stall_coeff)
    V_1g    = np.sqrt(1 / stall_coeff)
